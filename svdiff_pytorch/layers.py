...
class SVDEmbedding(nn.Embedding):
    ...
    def forward(self, x: torch.Tensor):
        if not self.done_svd:
            # this happens after loading the state dict
            self.perform_svd()
        weight_updated = self.U.to(x.device) @ torch.diag(F.relu(self.S.to(x.device) + self.scale * self.delta)) \
            @ self.Vh.to(x.device)
        return F.embedding(
...
class SVDLayerNorm(nn.LayerNorm):
    ...
    def forward(self, x: torch.Tensor):
        if not self.done_svd:
            ...
            self.perform_svd()
        weight_updated = (
            self.U.to(x.device, dtype=x.dtype)
            @ torch.diag(F.relu(self.S.to(x.device, dtype=x.dtype) + self.scale * self.delta))
            @ self.Vh.to(x.device, dtype=x.dtype)
        )
